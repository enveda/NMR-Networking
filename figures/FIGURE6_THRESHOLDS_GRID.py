...
from matplotlib.ticker import MultipleLocator, FuncFormatter, FixedLocator, FormatStrFormatter
...
def draw_grouped_bars_on_axis(
	...
	if as_percent:
		before_vals = [v * 100.0 for v in before_vals]
		after_vals = [v * 100.0 for v in after_vals]
	...
	# Y-axis formatting
	if as_percent:
		ax.yaxis.set_major_locator(FixedLocator([0, 25, 50, 75]))
...
def add_metric_label(ax: plt.Axes, text: str) -> None:
	ax.text(0.50, 0.96, text, transform=ax.transAxes, ha='center', va='top', fontsize=20,)

def compute_percentage_above_threshold_avg(
	reranked_annotations: List[Tuple[str, str, pd.DataFrame]],
	...
	print(f"[INFO] Computing percentage above threshold for AVG hybrid > {threshold} for k in {topk_values}")
	counts_before = {k: 0 for k in topk_values}
	counts_after = {k: 0 for k in topk_values}
	N = len(reranked_annotations)
	for idx, (file_name, exp_smiles, reranked_df) in enumerate(reranked_annotations):
		...
		for k in topk_values:
			...
			if before_mean > threshold:
				counts_before[k] += 1
			if after_mean > threshold:
				counts_after[k] += 1
	percent_before = {k: counts_before[k] / N for k in topk_values}
	percent_after = {k: counts_after[k] / N for k in topk_values}
	return percent_before, percent_after

def compute_percentage_above_threshold_max(
	reranked_annotations: List[Tuple[str, str, pd.DataFrame]],
	metric_score_column: str,
	topk_values: List[int],
	require_min_rows: int,
	...
	print(f"[INFO] Computing percentage above threshold for MAX hybrid > {threshold} for k in {topk_values}")
	counts_before = {k: 0 for k in topk_values}
	counts_after = {k: 0 for k in topk_values}
	N = len(reranked_annotations)
	for idx, (file_name, exp_smiles, reranked_df) in enumerate(reranked_annotations):
		...
		for k in topk_values:
			...
			if before_max > threshold:
				counts_before[k] += 1
			if after_max > threshold:
				counts_after[k] += 1
	percent_before = {k: counts_before[k] / N for k in topk_values}
	percent_after = {k: counts_after[k] / N for k in topk_values}
	return percent_before, percent_after

def compute_percentage_above_threshold_structural_efficiency(
	reranked_annotations: List[Tuple[str, str, pd.DataFrame]],
	metric_score_column: str,
	k_values: List[int],
	top100: int,
	...
	print(f"[INFO] Computing percentage above threshold for structural efficiency > {threshold} for top-k {k_values} vs top-{top100}")
	counts_before = {k: 0 for k in k_values}
	counts_after = {k: 0 for k in k_values}
	N = len(reranked_annotations)
	for idx, (file_name, exp_smiles, reranked_df) in enumerate(reranked_annotations):
		...
		for k in k_values:
			...
			if eff_before > threshold:
				counts_before[k] += 1
			if eff_after > threshold:
				counts_after[k] += 1
	percent_before = {k: counts_before[k] / N for k in k_values}
	percent_after = {k: counts_after[k] / N for k in k_values}
	return percent_before, percent_after
...
def main():
	...
	# Try to load experimental dict; if RDKit not available (or other error), skip peak filtering
	experimental_dict = None
	try:
		experimental_dict = load_experimental_dict(experimental_pkl_path)
	except ModuleNotFoundError as e:
		print(f"[WARN] Optional experimental dict requires missing module: {e}. Skipping min-peaks filtering.")
	except Exception as e:
		print(f"[WARN] Could not load experimental dict ({e}). Skipping min-peaks filtering.")

	# Filter by min peaks if experimental dict is available
	min_peaks = 15
	if experimental_dict is not None:
		...
		print("[INFO] Proceeding without min-peaks filtering.")

	# Combine low and high
	combined_annotations = low_annotations + high_annotations
	print(f"[INFO] Combined annotations: {len(combined_annotations)}")

	# Settings
	topk_values = [3, 10]
	require_min_rows = max(topk_values)
	avg_threshold = 0.5
	max_threshold = 0.5
	eff_threshold = 0.9

	# Compute percentages
	avg_pct_before, avg_pct_after = compute_percentage_above_threshold_avg(
		combined_annotations, metric_score_column, topk_values, require_min_rows, threshold=avg_threshold
	)
	max_pct_before, max_pct_after = compute_percentage_above_threshold_max(
		combined_annotations, metric_score_column, topk_values, require_min_rows, threshold=max_threshold
	)
	eff_pct_before, eff_pct_after = compute_percentage_above_threshold_structural_efficiency(
		combined_annotations, metric_score_column, k_values=topk_values, top100=100, threshold=eff_threshold
	)

	# Build 2x1 grid (vertical): Top-10 only, Average Hybrid over Structural Efficiency
	fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(5, 8.), sharex=False, sharey=False)
	fig.subplots_adjust(wspace=0.0, hspace=0.0)

	# Panel 1: Average Hybrid threshold percentage (Top-3 and Top-10)
	draw_grouped_bars_on_axis(
		axes[0], {3: avg_pct_before[3], 10: avg_pct_before[10]}, {3: avg_pct_after[3], 10: avg_pct_after[10]}, y_label="% > Thresh", y_lim=(0, 100), show_legend=False, as_percent=True
	)
	# axes[0].set_title("Top-10", fontsize=20)
	# Hide x-axis tick labels for the top panel
	axes[0].tick_params(axis='x', labelbottom=False)
	add_metric_label(axes[0], f"Avg. Hybrid > {avg_threshold}")

	# Panel 2: Structural Efficiency threshold percentage (Top-3 and Top-10)
	draw_grouped_bars_on_axis(
		axes[1], {3: eff_pct_before[3], 10: eff_pct_before[10]}, {3: eff_pct_after[3], 10: eff_pct_after[10]}, y_label="% > Thresh.", y_lim=(0, 100), show_legend=False, as_percent=True
	)
	add_metric_label(axes[1], f"$\\eta$ > {eff_threshold}")

	# Save figure
	out_dir = Path("/home/cailum.stienstra/HSQC_Models/Networking_HSQC/demo/figures/paper_figures/comparison_thresholds/")
	out_dir.mkdir(parents=True, exist_ok=True)
	out_path = out_dir / f"{low_composite}_pct_thr_TOP3_10_AVG{avg_threshold}_EFF{eff_threshold}_AUG20.png"
	plt.savefig(out_path, dpi=300, bbox_inches='tight')
	print(f"[INFO] Saved thresholds 2x1 grid figure: {out_path}")
	plt.show()
